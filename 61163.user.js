...
//        });
	...
	function customCSS(){
		...
		if(hideActionBarTooltips === 'true'){
			GM_addStyle('.actions span{display:none !important;}');
			GM_addStyle('.actions b{display:none !important;}');
	    }
		...
		.actions .ts-ret, .actions .ts-edt, .actions .ts-mute, .actions .ts-evernote{
			margin:0 1px 0 0;
		}
		.actions .ts-ret span, .actions .ts-edt span, .actions .ts-mute span, .actions .ts-evernote span{
			margin:0 0 0 4px;
		}
		.actions i:hover{
			opacity: 0.5;
...
//		});
		...
		$('#stream-items-id .stream-item').live('mouseenter', function(){
			...
			if(hideEvernote === 'false')
		    	$('.actions:not(:has(.ts-evernote))', $(this)).find('.action-fav-container').after(tsl);
			if(hideMute === 'false')
		    	$('.actions:not(:has(.ts-mute))', $(this)).find('.action-fav-container').after(mut);
		    if(hideRT === 'false')
				$('.actions:not(:has(.ts-ret)):has(.action-del-container:hidden)', $(this)).find('.action-fav-container').after(ret);
		    $('.actions:not(:has(.ts-edt)):has(.action-del-container:visible)', $(this)).find('.action-fav-container').after(edt);
	    ...
	    $('.ts-ret').live('mouseup', function(e){
	        ...
	        new twttr.widget.TweetDialog({
	            draggable:true,template:{
	            	title:"ReTweeting"
	            }, defaultContent:ts_content, origin:"TS-Retweet-button"
	        }).open().focus();
			$('.tweet-button').removeClass('disabled');
			$('.tweet-button').addClass('primary-btn');
			setTimeout(function(){$('.twttr-dialog-wrapper .twitter-anywhere-tweet-box-editor').selectRange(0, 0)}, 250);
	    });
